...
* Built in conditions
* Using messages instead of print
...
## Are you using 'print' to debug?

- It is a pain to add and remove the print statements
```{r, eval= TRUE, echo=TRUE}
add <- function (...) {
   print(paste("add:Parameters:", paste(..., collapse = ", ")))
   result <- sum(...)
   print(paste("add:returned:", result))
   result
   invisible()
}
add(1:10)
...
## Advanced Bonus Method

- Using restarts, you can create a handler where you do not need to wrap the call in suppressMessages()
- Wrap the function in withCallingHandlers and choose a handler
```{r, eval= TRUE, echo=TRUE}
add <- function (...) {
   withCallingHandlers( 
      {
      ...
      # comment the line below to see messages
...
- The buffer a variable called last.warning
- last.warning is:
    - a named list
...
```{r, eval= TRUE, echo=TRUE}
my_warning <- function() print(getwd())
options(warning.expression = quote(my_warning()))
warning("Test")
```
...
## Global Options for Errors

- error:
    - A function to handle unhandled error conditions
- showErrorCalls:
    - Show a summary of the call stack. Default: TRUE in interactive mode
- show.error.locations:
    - Print the source locations of errors
    - values: TRUE/FALSE/'top'/'bottom' or and integer
- show.error.messages:
    - Print error messages (TRUE/FALSE)
    - There is no suppressError() function
...
`%+%` <- function(lhs, rhs) {
   if (!is.integer(lhs) || !is.integer(rhs)){
      stop("parameters number be numeric")
   }
   lhs + rhs
}
tryCatch( { 5L %+% 4L },
...
## tryCatch: Warning handler defined
...
```
- No warning message and sum is not computed
